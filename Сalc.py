...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.b_plus.clicked.connect(lambda: self._operation("+")) #знаки
        self.b_minus.clicked.connect(lambda: self._operation("-"))
        self.b_umn.clicked.connect(lambda: self._operation("x"))
        self.b_razd.clicked.connect(lambda: self._operation("/"))
        self.b_result.clicked.connect(self._result)
        self.b_del.clicked.connect(self._del)

        self.b_1.clicked.connect(lambda: self._button("1")) #числа
    ...
    def _result(self):                          #Плав точка и деление на ноль
        ...
        if self.op == "/":
            if self.num_2 == 0:
                self.input.setText("Невозможно поделить")
            self.input.setText(drobn(self.num_1 / self.num_2))
